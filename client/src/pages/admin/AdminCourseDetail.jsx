  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className="text-foreground font-semibold font-bold truncate">{course?.title}</span>
      </div>
      ...
      {/* Course header */}
      <div className="bg-card text-card-foreground border rounded-xl shadow-sm rounded-3xl border border-border p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-border flex items-center justify-center shrink-0">
              <BookOpen className="h-6 w-6 text-primary" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-foreground font-semibold">{course?.title}</h1>
              {course?.description && (
          ...
          <Link
            ...
            className="shrink-0 p-2.5 text-muted-foreground hover:text-foreground font-semibold hover:bg-muted/80 rounded-xl transition-colors"
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl text-sm font-bold transition-all shadow-[0_0_15px_rgba(168,85,247,0.3)]"
          >
        ...
        {(!course?.modules || course.modules.length === 0) ? (
          <div className="bg-card text-card-foreground border rounded-xl shadow-sm rounded-2xl border border-border p-10 text-center">
            <Layers className="h-10 w-10 text-muted-foreground mx-auto mb-3" />
            <p className="text-muted-foreground font-medium">No modules yet</p>
            <p className="text-muted-foreground text-sm mt-1">Add your first module to get started</p>
          </div>
        ) : (
          course.modules.map((mod, index) => (
            <div key={mod.id} className="bg-card text-card-foreground border rounded-xl shadow-sm rounded-2xl border border-border overflow-hidden">
              {/* Module header */}
              <div className="flex items-center justify-between p-4 gap-3">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-foreground font-semibold text-sm truncate">{mod.title}</p>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <button
                      ...
                      disabled={index === 0}
                      className="p-1 text-muted-foreground hover:text-muted-foreground disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                      title="Move up"
                    >
                  ...
                  <Link
                    ...
                    state={{ courseId: id }}
                    className="px-3 py-1.5 text-[11px] font-bold text-primary bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 rounded-lg transition-colors uppercase tracking-widest whitespace-nowrap"
                  >
                    + Lesson
                  </Link>
                  <button
              ...
              {expandedModules[mod.id] && mod.lessons && mod.lessons.length > 0 && (
                <div className="border-t border-border divide-y divide-white/5">
                  {mod.lessons.map((lesson) => (
                    <div key={lesson.id} className="flex items-center justify-between px-4 py-3 pl-14 hover:bg-muted transition-colors">
                      <div className="flex items-center gap-2 min-w-0">
                        <FileText className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                        <span className="text-sm text-muted-foreground truncate">{lesson.title}</span>
                        {!lesson.isPublished && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground border border-border font-bold uppercase tracking-wide shrink-0">
                            Draft
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 shrink-0">
                        <Link
                          ...
                          className="p-1.5 text-muted-foreground hover:text-foreground font-semibold hover:bg-muted/80 rounded-lg transition-colors"
                        >
