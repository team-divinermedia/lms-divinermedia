...
const statusConfig = {
  pending: { label: 'Pending', bg: 'bg-yellow-500/10', text: 'text-yellow-300', border: 'border-yellow-500/30' },
  approved: { label: 'Approved', bg: 'bg-emerald-500/10', text: 'text-emerald-300', border: 'border-emerald-500/30' },
  rejected: { label: 'Rejected', bg: 'bg-red-500/10', text: 'text-red-300', border: 'border-red-500/30' },
  revision: { label: 'Needs Revision', bg: 'bg-orange-500/10', text: 'text-orange-300', border: 'border-orange-500/30' },
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-white/10">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">My Portfolio</h1>
          <p className="text-gray-400 mt-1 font-medium">{proofs.length} proof{proofs.length !== 1 ? 's' : ''} of work</p>
        </div>
        <Link
          to="/proofs/new"
      ...
      {proofs.length === 0 ? (
        <div className="glass-panel rounded-3xl border border-white/10 p-12 text-center">
          <div className="w-16 h-16 bg-white/5 rounded-3xl flex items-center justify-center mx-auto mb-4 ring-1 ring-white/10">
            <Award className="h-8 w-8 text-gray-500" />
          </div>
          <h3 className="text-lg font-bold text-white mb-1">No proofs yet</h3>
          <p className="text-gray-500 text-sm mb-4">Upload your work to build your portfolio</p>
          <Link
            to="/proofs/new"
            className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-xl text-sm font-bold transition-all shadow-[0_0_15px_rgba(168,85,247,0.3)]"
          >
            <Plus className="h-4 w-4" />
            Upload Your Work
          </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {proofs.map((proof) => {
            ...
            return (
              <div key={pId} className="glass-panel rounded-2xl border border-white/10 overflow-hidden hover:border-white/20 hover:-translate-y-1 transition-all shadow-lg group">
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs text-gray-500 capitalize font-medium">{proof.type}</span>
                  </div>
                  <h3 className="font-bold text-white">{proof.title}</h3>
                  {proof.description && (
                    <p className="text-sm text-gray-500 mt-1 line-clamp-2 leading-relaxed">{proof.description}</p>
                  )}
                  {proof.lessonTitle && (
                    <p className="text-xs text-gray-600 mt-2 font-medium">Lesson: {proof.lessonTitle}</p>
                  )}
                  {proof.adminNote && (
                    <div className="mt-3 p-2.5 bg-black/30 rounded-xl border border-white/5">
                      <p className="text-xs text-gray-400 leading-relaxed">{proof.adminNote}</p>
                    </div>
                  )}
                  {(proof.fileUrl || proof.url) && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-sm text-purple-400 hover:text-purple-300 mt-3 font-medium transition-colors"
                    >
