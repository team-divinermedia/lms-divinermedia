    ...
    <div className="space-y-8 relative z-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 pb-2 border-b border-border">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-foreground to-muted-foreground tracking-tight">Manage Courses</h1>
          <p className="text-muted-foreground mt-1 font-medium">{courses.length} module{courses.length !== 1 ? 's' : ''} deployed</p>
        </div>
        <Link
          to="/admin/courses/new"
          className="inline-flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl text-sm font-bold tracking-wide uppercase shadow-[0_0_20px_rgba(219,39,119,0.3)] hover:shadow-[0_0_30px_rgba(219,39,119,0.5)] transition-all hover:-translate-y-0.5"
        >
      ...
      {courses.length === 0 ? (
        <div className="bg-card text-card-foreground border rounded-xl shadow-sm rounded-[2.5rem] border border-border p-16 text-center">
          ...
          <h3 className="text-2xl font-bold text-foreground font-semibold mb-2 tracking-tight">No courses active</h3>
          <p className="text-muted-foreground text-sm mb-8 max-w-sm mx-auto">Create your first training module to begin curating the intern curriculum.</p>
          <Link
            to="/admin/courses/new"
            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold rounded-xl text-sm font-bold tracking-wide uppercase transition-all shadow-lg hover:shadow-pink-500/50"
          >
        ...
        <div className="grid grid-cols-1 gap-4">
          {courses.map((course) => {
            ...
            return (
              <div key={cId} className="bg-card text-card-foreground border rounded-xl shadow-sm rounded-3xl border border-border p-5 hover:border-border transition-all duration-300 shadow-lg group">
                ...
                <div className="flex items-start gap-4">
                  {course.thumbnail ? (
                    ...
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-border flex items-center justify-center shrink-0 shadow-inner group-hover:from-purple-500/30 transition-colors">
                      <BookOpen className="h-6 w-6 text-primary" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="text-base font-bold text-foreground font-semibold truncate tracking-tight leading-tight">{course.title}</h3>
                ...
                <div className="flex items-center justify-between gap-2 flex-wrap mt-4 pt-4 border-t border-border">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
                    <span className="flex items-center gap-1.5 bg-muted px-2.5 py-1.5 rounded-lg border border-border">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <Link
                      ...
                      className="px-3 py-1.5 text-[11px] font-bold text-primary bg-pink-500/10 hover:bg-pink-500/20 border border-pink-500/20 rounded-lg transition-colors uppercase tracking-widest"
                    >
                      Manage
                    </Link>
                    <Link
                      to={`/admin/courses/${cId}/edit`}
                      className="p-2 text-muted-foreground hover:text-foreground font-semibold hover:bg-muted/80 rounded-lg transition-colors"
                    >
