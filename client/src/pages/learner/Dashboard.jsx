  ...
  return (
    <div className="space-y-10">
      <div className="relative">
        ...
        <p className="text-purple-200/70 mt-2 font-medium">Continue your learning journey through the cosmos.</p>
      </div>

      {lastLesson && (
        <Link
          to={`/lessons/${lastLesson._id || lastLesson.id}`}
          className="block relative overflow-hidden bg-gradient-to-r from-purple-900/80 to-cyan-900/80 backdrop-blur-xl border border-white/20 rounded-3xl p-8 text-white hover:from-purple-800/90 hover:to-cyan-800/90 hover:border-purple-400/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.4)] transition-all duration-500 group"
        >
          ...
          <div className="relative flex items-center justify-between z-10">
            <div className="flex items-center gap-5">
              <div className="h-16 w-16 bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:bg-white/20 transition-all duration-300">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-white drop-shadow-md">{lastLesson.title}</h3>
                {lastLesson.courseName && (
                  <p className="text-purple-200/80 mt-1 font-medium">{lastLesson.courseName}</p>
                )}
              </div>
            </div>
            <div className="h-12 w-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-white/10 transition-colors">
          ...
          {lastLesson.progress !== undefined && (
            <div className="mt-8 relative z-10">
              <div className="flex justify-between text-sm mb-2 font-medium">
                <span className="text-purple-200/90">Course Progress</span>
                <span className="text-cyan-300">{Math.round(lastLesson.progress)}%</span>
              </div>
              <div className="w-full bg-black/40 rounded-full h-2.5 shadow-inner overflow-hidden border border-white/5">
                <div
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white drop-shadow-sm">My Enrolled Courses</h2>
          <Link to="/courses" className="text-sm text-cyan-400 hover:text-cyan-300 font-semibold px-4 py-2 rounded-lg hover:bg-cyan-500/10 transition-colors">
            View all directory
          </Link>
        </div>

        {courses.length === 0 ? (
          <div className="glass-panel rounded-3xl p-16 text-center shadow-lg pointer-events-none">
            ...
            <p className="text-purple-200/60 font-medium">Your enrolled modules will appear in this sector.</p>
          </div>
        ) : (
      ...
      {recentActivity.length > 0 && (
        <div className="pt-4">
          <h2 className="text-xl font-bold text-white mb-6 drop-shadow-sm">Recent Transmissions</h2>
          <div className="glass-panel rounded-3xl border border-white/10 divide-y divide-white/5">
            {recentActivity.slice(0, 5).map((activity, idx) => (
              <div key={idx} className="px-6 py-5 flex items-center gap-5 hover:bg-white/5 transition-colors first:rounded-t-3xl last:rounded-b-3xl">
                <div className="h-12 w-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 shadow-inner">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-base font-medium text-gray-200 truncate">{activity.description || activity.title}</p>
                  <p className="text-xs text-purple-300/60 font-medium mt-1 uppercase tracking-wider">
